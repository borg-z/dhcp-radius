...
srv = Client(server="127.0.0.1", secret=b"Kah3choteereethiejeimaeziecumi", dict=Dictionary("dictionary")) #radius
...
#		print (body)
		...
		ip = body[5]
		vlan = getoutput("grep " + ip + " /var/log/dhcpd.log | grep exp -B1 | tail -n2").split('\n')[0].split()[9]
		status = 'Stop'
		getid_by_vlan(vlan, ip, status)
...
def getid_by_vlan(vlan, ip, status):
	resp = requests.get('https://site.com/monitoring/opticalaccountidbyvlan?vlan='+vlan)
